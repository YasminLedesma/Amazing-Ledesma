...
let todaLaInfo; //todo, toda la info--- donde esta TODO

fetch("https://mindhub-xj03.onrender.com/api/amazing")
.then(data => data.json())
.then(data => {
    todaLaInfo = data
    renderTemplate (createCards(todaLaInfo.events), $cards)
...
function filtroCruzado(evento){
  let checkbuttons = document.querySelectorAll(".form-check-input")
    const filterPerFind = searchFilter ($search, todaLaInfo.events)
